...
namespace oraiBarnanenak
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int afa = 27;
            foreach (var arak in Monitorok)
            {
                int eredmeny = arak.Ar * afa;
            ...
            Console.WriteLine("---------------------------------------------------------------------------------");
            foreach (var osszAr in Monitorok)
            {
                int osszes = osszAr.Ar * afa * osszAr.Raktaron;
            ...
            using (StreamWriter sw = new StreamWriter(@"..\..\..\src\50-felett.txt", append: false))
            {
                ...
                foreach (var monitor in Monitorok)
                {
                    if (monitor.Ar > 50000)
                    {
                        sw.WriteLine($"{monitor.Nev.ToUpper()};{monitor.Tipus.ToUpper()};{monitor.Meret};{monitor.Raktaron},{monitor.Ar}EZER");
                    }
                    Console.WriteLine(monitor.Ar >50000);
                }
            }

            //5. Egy vevő keresi a HP EliteDisplay E242 monitort. Írd ki neki a képernyőre, hogy hány darab ilyen van a készleten.
            //Ha nincs a készleten, ajánlj neki egy olyan monitort, aminek az ára az átlaghoz fölülről közelít. Ehhez használd
            //az átlagszámító függvényt (később lesz feladat). 
            foreach (var keres in Monitorok)
            {
                if (keres.Nev == "HP EliteDisplay E242")
                {
                    Console.WriteLine(keres.Raktaron);
                }
                else
                {
                    Console.WriteLine(keres.Nev();
                    Console.WriteLine(keres.Nev();
                }
            }
